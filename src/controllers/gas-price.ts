...
export const getGasPrice = async (request: CustomRequest, response: Response) => {
    try {
        ...
        console.log('👉🏻 Line 57 : ', provider, DATA_PROVIDER_MAPPING.COVALENTHQ);
        ...
        switch (provider) {
            ...
            case DATA_PROVIDER_MAPPING.COVALENTHQ: {
                ...
                result = await client.BaseService.getGasPrices(network, "erc20");
                if (!result?.data?.base_fee) {
                    throw new Error(HTTP_RESPONSE_MESSAGES.NO_DATA_FOUND);
                }
                result = transformGasDetails(result?.data?.base_fee, provider);
                break;
            }
            default:
