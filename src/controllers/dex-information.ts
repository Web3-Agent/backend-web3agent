...
export const getDexInformation = async (
    ...
    try {
        ...
        let {
            provider,
            query: { network = "ether", sortKey = "pools", pageSize = 10 },
        } = body;
        switch (provider) {
            case DATA_PROVIDER_MAPPING.DEX_TOOLS: {
                const { data } = await axios.get(
                    `https://api.dextools.io/v2/dex/${network}/?sort=${sortKey}&order=desc&pageSize=${pageSize}`,
                ...
                return response.status(200).json({
                    success: true,
                    message: HTTP_RESPONSE_MESSAGES.ACTION_SUCCESS,
                    data: data?.data,
                    request: body,
                });
            }
...
export default { getDexInformation };
